 ...
 let chanceDeck = [
  {
    ...
    buttonText: "Okay",
    result: function() {
      // chanceDeck.splice(chanceDeck.indexOf(cardDrawn), 1);
      // chanceDeckDiscarded.push(cardDrawn);
  ...
  jailFreeChance = {
    text: 'The government has decided that you business ventures are "too big to fail" and is prepared to issue a pardon. You may use this pardon to get out of jail free. This card may be kept until needed or traded/sold.',
    buttonText: 'Okay',
    owner: null,
    result: function() {
  ...
  },
  {
    ...
    },
    {
      ...
      result: function() {
        ...
        let player = gameStatus.currentPlayerTurn;
        let moneyOwed = 0;
        for (property of player.properties) {
    ...
    {
      text: "Your building loan matures, and for some reason, you get $150.",
      buttonText: "Okay?",
      result: function() {
        // chanceDeck.splice(chanceDeck.indexOf(cardDrawn), 1);
        // chanceDeckDiscarded.push(cardDrawn);
...
function landOn(space) {
  if (space.type === "coloredProperty") {
    ...
    let player = gameStatus.currentPlayerTurn;
    // CHANGE BACK TO displayChance() WHEN COMMUNITY CHEST DECK IS COMPLETED.
    // displayChance();
...
$(document).on("click", ".board-details > .take-a-chance", function() {
  if (chanceDeck.length === 0) {
    chanceDeck = chanceDeckDiscarded;
...
$(document).on("click", ".board-details > .look-in-chest", function() {
  if (communityChestDeck.length === 0) {
    communityChestDeck = communityChestDeckDiscarded;
    communityChestDeckDiscarded.splice(0, communityChestDeckDiscarded.length);
  }
  cardDrawn = communityChestDeck[Math.floor(Math.random() * communityChestDeck.length)];
  console.log(cardDrawn);
...
$(document).on('click', ".board-details > .btn-cc-result", function() {
  // communityChestDeck.splice(communityChestDeck.indexOf(cardDrawn), 1);
  // communityChestDeckDiscarded.push(cardDrawn);
  cardDrawn.result();
});
